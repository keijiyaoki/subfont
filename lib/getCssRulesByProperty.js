...
function getCssRulesByProperty(inlineCssMap, properties, cssSource, existingPredicates) {
  let parseTree;
  if (!Array.isArray(properties)) {
    throw new Error('properties argument must be an array');
  }
  if (Array.isArray(cssSource)) {
    const dataHref = cssSource[0];
    parseTree = inlineCssMap.get(dataHref);
    cssSource = (parseTree) ? 'parse done' : cssSource;
  }
  ...
  parseTree = parseTree || postcss.parse(cssSource);
