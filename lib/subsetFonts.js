...
function getSharedCssCache() {
  const keys = [];
  const list = [];

  const toFontBuffer = (subsets) => {
    return subsets.woff2 || subsets.woff || subsets.truetype || subsets.opentype;
  };

  const set = (subsetFontUsages, text, asset) => {
    const subsets = subsetFontUsages.map((fontUsage) => fontUsage.subsets);
    keys.push(subsets);
    list.push({ text, asset });
  };

  const get = (subsetFontUsages) => {
    const subsets = subsetFontUsages.map((fontUsage) => fontUsage.subsets);
    const index = keys.findIndex((key) => {
      return ((key.length === subsets.length) &&
              !key.some((element, index) => {
                return toFontBuffer(element) !== toFontBuffer(subsets[index]);
              }));
    });
...
async function subsetFonts(
  ...
  try {
    for (const htmlAsset of htmlAssets) {
      ...
      if (accumulatedFontFaceDeclarations.length > 0) {
        ...
        if (htmlAssetTextsWithProps.length === htmlAssets.length / 2) {
          console.log(`traced font texts: ${htmlAssetTextsWithProps.length}/${htmlAssets.length}`);
        }
      }
    }
  } finally {
  ...
  console.log(`traced font texts: total ${htmlAssetTextsWithProps.length}`);
  ...
  for (const htmlAssetTextsWithPropsEntry of htmlAssetTextsWithProps) {
    ...
    delete htmlAssetTextsWithPropsEntry.textByProps;
...
These glyphs are used on your site, but they don't exist in the font you applied to them:`;
  ...
  const sharedCssCache = getSharedCssCache();
  for (const {
    htmlAsset,
    fontUsages,
    accumulatedFontFaceDeclarations,
    ...
    const sharedCss = sharedCssCache.get(subsetFontUsages);
    let subsetCssText = ((sharedCss) ?
                         sharedCss.text :
                         getFontUsageStylesheet(subsetFontUsages));
    const subsetCssTextOrigin = subsetCssText;
    ...
    if (!sharedCss) {
      ...
      if (htmlAssetTextsWithProps.globalFontUsages) {
        sharedCssCache.set(subsetFontUsages, subsetCssTextOrigin, cssAsset);
      }
    }
